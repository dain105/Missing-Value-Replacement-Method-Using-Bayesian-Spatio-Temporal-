...
setwd("z:/Lab/기상환경/NIMR/data/농진청")
list.files()

agri.stat<-read.csv("지리정보_강원+전남.csv",header=T)

setwd("z:/Lab/기상환경/NIMR/data/농진청/강원도")
...
setwd("z:/Lab/기상환경/NIMR/data/농진청/전남")
l.gangwon<-list.files()
...
setwd("z:/Lab/기상환경/NIMR/data/기상청")
...
#강원도 태백시 매봉산 730개
#강원도 삼척시 미로면 730개
#강원도_정선군_임계면 	1095 obs. of  10 variablez:
...
d.alt<-read.table("z:/Lab/기상환경/NIMR/data/Digital_grd.dat")
names(d.alt)<-c("x","y","lat","lon","alt","a","b","c")

d.alt <- d.alt[d.alt$lat>37 & d.alt$lon<127.3,]
t1<-unique(DataVal[,c("id","lon","lat")])
...
#   print(i)
  ...
  set.seed(12343)
  m_gpo <- spT.Gibbs(formula=temp ~ lon + lat + alt,
  ...
  plot(m_gpo)
  summary(m_gpo)
  autocorr.diag(as.mcmc(m_gpo))

  pred.gpo <- predict(m_gpo, newdata=DataVal, newcoords=pred.coords[,2:3], type="spatial", tol.dist=0.05)
  # pred.gp$Median
  
  
  gp.mtx[i,]<-spT.validation(as.numeric(paste(DataVal$temp)),c(pred.gpo$Median))
