...
tail(u.alt)
dim(u.alt)
pred.coords<-unique(DataVal[,c("id","lon","lat")])
dim(pred.coords)
dim(DataVal)
33580/23
...
# spatial.decay<-spatial.decay<-spT.decay(distribution="FIXED", value=0.006)
  ...
  gp.mtx <- matrix(rep(NA, 7*13),ncol=7)
  para.spatial<-seq(0.04,0.10,0.005)
  length(para.spatial)
  ...
  #################distribution=Gamm(2,1)#################
  set.seed(12343)
  m_gpo <- spT.Gibbs(formula=temp ~ lon + lat,
...
#################distribution=fixed#################
  ...
  nad<-as.matrix(subset(DataFit, DataFit$prec==is.na(DataFit$prec)))
  nad<-as.matrix(subset(DataFit, DataFit$temp==is.na(DataFit$temp)))
  nad<-as.matrix(subset(DataFit, DataFit$ws==is.na(DataFit$ws)))
  nad<-as.matrix(subset(DataFit, DataFit$rhc==is.na(DataFit$rh)))
  nad<-as.matrix(subset(DataFit, DataFit$prec==is.na(DataFit$prec)))
